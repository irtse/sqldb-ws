		...
		for _, schema := range schemas {
			...
			for _, v := range schema.Fields {
				if utils.ToMap(res[0]["schema"])[v.Name] == nil {
					...
					res[0]["order"] = append(res[0]["order"].([]interface{}), v.Name)
				}
			}
...
func (s *ViewService) TransformToView(schemas []*models.SchemaModel, record utils.Record, multiple bool, schema *models.SchemaModel, domainParams utils.Params,
	...
	if line, ok := domainParams.Get(utils.RootFilterLine); ok {
		...
		if val, operator, separator := connector.GetFieldInInjection(line, "type"); val != "" {
			...
			fmt.Println("ssdss", len(rschemas))
			schemas = rschemas
		}
...
func (s *ViewService) getFilterDetails(record utils.Record, schema *models.SchemaModel) (string, string, string) {
	...
	sqlFilter, view, _, dir, _, _, _ := filterserv.NewFilterService(s.Domain).GetFilterForQuery(
