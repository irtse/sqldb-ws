			...
			if dest, ok := r["from_"+ds.DestTableDBField]; ok && dest != nil {
				...
				if schFrom.Name == ds.DBUser.Name && ff.Name == utils.SpecialIDParam {
					...
					if rr, err := s.Domain.GetDb().ClearQueryFilter().SelectQueryWithRestriction(schFrom.Name,
						utils.ToListAnonymized(filter.NewFilterService(s.Domain).RestrictionByEntityUser(schFrom, []string{}, true, false, false)), false); err == nil && len(rr) > 0 {
				...
				if schFrom.Name == ds.DBUser.Name {
					...
					if rr, err := s.Domain.GetDb().ClearQueryFilter().SelectQueryWithRestriction(schFrom.Name,
						utils.ToListAnonymized(filter.NewFilterService(s.Domain).RestrictionByEntityUser(schFrom, []string{}, true, false, false)),
